...
// Hangfire recurring job — đọc OutboxMessages của EnrollmentManagement BC,
// deserialize thành đúng IDomainEvent, publish qua IPublisher (MediatR in-process bus).
public class EnrollmentOutboxProcessor {
    ...
    private readonly ILogger<EnrollmentOutboxProcessor> _logger;

    public EnrollmentOutboxProcessor(
        EnrollmentDbContext context,
        IPublisher publisher,
        ILogger<EnrollmentOutboxProcessor> logger) {
        _context = context;
        ...
        foreach (var message in messages) {
            try {
                ...
                _logger.LogInformation(
                    "Processed Enrollment OutboxMessage {Id}, type {Type}.",
                    message.Id, eventType.Name);
            } catch (Exception ex) {
                message.MarkFailed(ex.Message);
                _logger.LogError(ex,
                    "Failed to process Enrollment OutboxMessage {Id}.", message.Id);
            }
        }
